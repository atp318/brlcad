...
gltf_read(struct gcv_context *context, const struct gcv_opts *UNUSED(gcv_options), const void *UNUSED(options_data), const char *source_path)
{
	...
	*/
	...
	int num_face_values;
	int num_norm_values;
	set_array_sizes(model, num_vert_values, num_face_values, num_norm_values);
	int *faces = new int[num_face_values];
	double *vertices = new double[num_vert_values * 3];
	double *normals = new double[num_norm_values * 3];
	insert_vector_faces(model, vertices, faces, normals);
	rt_wdb *outfp = context->dbip->dbi_wdbp;
	std::string title = "gltf conversion from" + input_filename;
	mk_id(outfp, title.c_str());
	mk_bot(outfp, "Mesh", RT_BOT_SURFACE, RT_BOT_UNORIENTED, 0, num_vert_values / 3, num_face_values / 3, vertices, faces, (fastf_t *)NULL, (struct bu_bitv *)NULL);
	//output_to_bot();
	return 1;
}
